...
- **Mastery is never signaled by color alone.** The `/cloud` view pairs each word's mastery color with a 4-pip filled-count meter (`MasteryPips` in `CloudClient.js`). Rationale: the New/Mastered color pair (rose/emerald) is exactly the red-green colorblind confusion pair, and the previous hover-only tooltip never fired on touch, which this app is mostly used on.
- **Generating a word set saves it as a deck immediately**, not after a separate "save" step — full deck functionality (review, amplify, etc.) is available the moment words appear. "Add more" and topic-picks extend the already-saved deck via the amplify endpoint rather than accumulating unsaved state client-side.
