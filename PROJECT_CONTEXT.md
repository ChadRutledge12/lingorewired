...
- **Onboarding** — `app/page.js` + `app/HomeClient.js`: questionnaire → Claude-generated word batch. Cumulative flow (new decks prefill from the most recent deck's profile, with per-field edit). Placement calibration step (`components/Calibration.js`, `lib/calibrationWords.js`) runs before first generation — see NEXT_TASK.md, currently uncommitted.
