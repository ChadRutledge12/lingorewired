...
// Slow/enunciated playback rate — deliberately slow so beginners can catch
// every syllable, but not so slow it sounds robotic.
export const SLOW_RATE = 0.6

// "Read it slowly" is an app-wide preference, not a per-button mode: someone
// who needs slow audio needs it on the story player and the translate screen
// too, not only on flashcards. Read at speak() time so every existing call
// site honors it without threading a prop down to each one.
export const SLOW_SPEECH_KEY = 'lingorewired:slowSpeech'

export function isSlowSpeechEnabled() {
  if (typeof window === 'undefined') return false
  return window.localStorage.getItem(SLOW_SPEECH_KEY) === 'true'
}
...
// `rate` (optional) forces the speech rate for this one call — a turtle button
// asking for slow audio regardless of the saved preference. Without it, the
// app-wide slow-speech preference decides.
export async function speak(text, gender = 'female', { onEnd, rate } = {}) {
  if (!isSpeechSupported() || !text) { onEnd?.(); return }
  const effectiveRate = rate ?? (isSlowSpeechEnabled() ? SLOW_RATE : undefined)
  ...
  playFromQueue(gen, voice, confident, gender, effectiveRate)
}
