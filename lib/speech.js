...
// prefer network/cloud-backed voices (localService: false) over on-device
// ones since they're usually higher quality. If we can't confidently
// classify any voice as the requested gender, fall back to any Spanish
// voice rather than guessing wrong — and say so via `confident: false`, so
// the caller knows this wasn't a real match.
function pickVoice(voices, gender) {
  ...
  if (matching.length > 0) {
    return { voice: matching.find((v) => !v.localService) || matching[0], confident: true }
  }
  return { voice: spanish.find((v) => !v.localService) || spanish[0] || null, confident: false }
}

// Only used as a last resort, when the browser has no voice we can
// confidently tell apart by gender — i.e. "male" and "female" would
// otherwise play the identical voice. The Web Speech API's pitch shift
// isn't formant-preserving, so pushing pitch *up* tends to sound strained
// ("chipmunk") much faster than pushing it *down* sounds merely deeper —
// keep the shift small and asymmetric to stay on the tolerable side of that.
const FALLBACK_PITCH = { female: 1.08, male: 0.95 }
...
export async function speak(text, gender = 'female') {
