...
// A story can be extended ("Continue the story") only until it reaches this
// many sentences, so it doesn't grow without bound (each extend also costs a
// comprehension-quiz regeneration and lengthens the TTS/quiz). Because each
// extend adds a fixed ~short chunk, this naturally allows FEWER extends for a
// reading that started long than one that started short — i.e. the number of
// extends depends on the story's length, as intended.
export const MAX_READING_SENTENCES = 24

// Whether a reading of this many sentences can still be extended.
export function canExtendReading(sentenceCount) {
  return (sentenceCount || 0) < MAX_READING_SENTENCES
}
