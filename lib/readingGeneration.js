...
export function buildReadingPrompt(targets, { profile, scenario, length = 'short' } = {}) {
  ...
  return `You are a Spanish teacher writing a short reading for a learner.
...
export function pickReadingTargets(cards, nowIso, cap = 12) {
  ...
  return [...due, ...notDue].slice(0, cap).map((c) => ({ word: c.word, translation: c.translation }))
}
